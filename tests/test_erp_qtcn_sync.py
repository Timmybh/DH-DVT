...
def db(monkeypatch):
    ...
    monkeypatch.setattr(tps, "write_audit", lambda action, **kw: None)
    monkeypatch.setattr(svc, "write_audit", lambda action, **kw: None)
...
def test_one_style_failure_does_not_block_other_styles(db, monkeypatch):
    ...
    def flaky(db_, user, run_id, item, congdoan_catalog, thietbi_catalog):
        calls["n"] += 1
        if calls["n"] == 1:
            raise RuntimeError("boom")
        return real_apply_one(db_, user, run_id, item, congdoan_catalog, thietbi_catalog)
