...
import { RevenueDetailData, Tile } from "../api/client";
...
function MiniBar({ value, expected }: { value: number | null; expected: number }) {
  return (
...
/** Phân tích chi tiết doanh thu — CHỈ hiển thị trong drill-down (không nằm trên dashboard chính). */
export default function RevenueDetail({ data, today }: Props) {
  ...
  const chart = data.trend.map((p) => ({ day: Number(p.date.slice(8, 10)), date: p.date, "Kế hoạch": p.plan, "Thực hiện": p.actual }));

  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Gauge label="Ngày gần nhất" sublabel={data.day ? dateVi(data.day.date) : "Chưa có số liệu"} pct={data.day?.pct ?? null} expected={90}>
          <TileBody tile={data.day ?? empty} unit={data.unit} />
        </Gauge>
        <Gauge label={`Tháng ${m}`} sublabel={`Thời gian đã qua ${data.elapsed_pct.toFixed(0)}%`} pct={data.month_tile.pct} expected={data.elapsed_pct}>
          <TileBody tile={data.month_tile} unit={data.unit} />
        </Gauge>
        <Gauge label={`Năm ${y}`} sublabel={`Thời gian đã qua ${yearElapsed.toFixed(0)}%`} pct={data.year_tile.pct} expected={yearElapsed}>
          <TileBody tile={data.year_tile} unit={data.unit} />
        </Gauge>
      </div>

      <div>
        <p className="mb-2 text-xs font-semibold text-slate-500">Kế hoạch vs Thực hiện theo ngày — tháng {m}/{y}</p>
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={chart} margin={{ left: -10, right: 4 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
            <XAxis dataKey="day" tick={{ fontSize: 11 }} />
            <YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => (v >= 1000 ? `${Math.round(v / 1000)}k` : String(v))} />
            <Tooltip formatter={(v) => num(v as number)} labelFormatter={(_, p) => (p?.[0]?.payload?.date ? dateVi(p[0].payload.date) : "")} />
            <Legend />
            <Bar dataKey="Kế hoạch" fill="#c7d2fe" radius={[3, 3, 0, 0]} />
            <Bar dataKey="Thực hiện" fill="#4f46e5" radius={[3, 3, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
          ...
          <tbody>
            {data.by_factory.map((f) => (
              <tr key={f.code} className="border-b border-slate-50">
                ...
                <td className="py-2 pl-4"><MiniBar value={f.month_pct} expected={data.elapsed_pct} /></td>
                <td className="py-2 pl-4"><MiniBar value={f.year_pct} expected={yearElapsed} /></td>
              </tr>
            ))}
          </tbody>
        </table>
